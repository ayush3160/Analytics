...
async function apicalls() {
    await fetch('http://localhost:3007/track/sessionData')
      ...
      {
        console.log(data);
        let dateWiseResult = data.dateWiseResult;
        topSessions = data.topSession;
        browserPercentage = data.browserPercentage;

        console.log(data.dateWiseResult);
    
        Object.keys(dateWiseResult).forEach(element => {
            dateWise.push(dateWiseResult[element]);
        });
...
setTimeout(() => {
    ...
    $(document).ready(function() {
        Object.keys(topSessions).forEach(element =>{
    
            $("#viewsPerMin").append(`<tr>
                <td>
                    <a href="javascript:void(0);" class="text-muted">${topSessions[element]._id}</a>
                </td>
                <td>${topSessions[element].count}</td>
                </tr>`)
            })
        });
        // <td>87.5%</td>
    ! function(i) {
        ...
        e.prototype.initCharts = function() {
            ...
            var e = new Date,
                t = function(e, t) {
                    for (var a = new Date(t, e, 1), o = [], r = 0; a.getMonth() === e && r < 7;) {
                        var s = new Date(a);
            ...
            r = {
                ...
                labels: ["Chrome", "Firefox", "Safari", "Opera", "Edge", "Explorer", "other"],
                plotOptions: {
                    radar: {
                        size: 130,
                        polygons: {
